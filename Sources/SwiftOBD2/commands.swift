...
public struct CommandProperties {
    ...
    public func decode(data: Data, isMetric: MeasurementUnits = .metric) -> DecodeResult? {
        switch decoder {
        case .pid:
            return .none
        case .status:
            return .statusResult(decodeStatus(data))
        case .uas0x09:
            return decodeSpeed(data, isMetric)
        case .uas0x07:
            guard let measurement = decodeUAS(data, id: 0x07) else { return nil }
            return .measurementResult(measurement)
        case .temp:
            guard let measurement = decodeTemp(data) else { return nil }
            return .measurementResult(measurement)
        case .percent:
            guard let percent = percent(data) else { return nil }
            return .measurementResult(percent)
        case .currentCentered:
            guard let currentCentered = currentCentered(data) else { return nil }
            return .measurementResult(currentCentered)
        case .airStatus:
            guard let airStatus = currentCentered(data) else { return nil }
            return .measurementResult(airStatus)
        case .singleDTC:
            guard let dtc = singleDtc(data) else { return nil }
            return .troubleCode([dtc])

        case .fuelStatus:
            guard let fuelStatus = fuelStatus(data) else { return nil }
            return .stringResult(fuelStatus)

        case .percentCentered:
            guard let percentCentered = percent(data) else { return nil }
            return .measurementResult(percentCentered)
        case .fuelPressure:
            guard let fuelPressure = fuelPressure(data) else { return nil }
            return .measurementResult(fuelPressure)
        case .pressure:
            guard let pressure = pressure(data) else { return nil }
            return .measurementResult(pressure)
        case .timingAdvance:
            guard let timingAdvance = timingAdvance(data) else { return nil }
            return .measurementResult(timingAdvance)
            // TODO : add obd Compliance return type

        case .obdCompliance:
            guard let obdCompliance = obdCompliance(data) else { return nil }
            return .stringResult(obdCompliance)

        case .o2SensorsAlt:
            guard let o2SensorsAlt = o2SensorsAlt(data) else { return nil }
            return .stringResult(o2SensorsAlt)

        case .uas0x12:
            guard let uasValue =  decodeUAS(data, id: 0x12) else { return nil }
            return .measurementResult(uasValue)

        case .o2Sensors:
            guard let o2Sensors = o2Sensors(data) else { return nil }
            return .stringResult(o2Sensors)

        case .sensorVoltage:
            guard let voltage = voltage(data) else { return nil }
        ...
        case .uas0x19:
            guard let uasValue = decodeUAS(data, id: 0x19) else { return nil }
            return .measurementResult(uasValue)
        case .uas0x1B:
            guard let uasValue = decodeUAS(data, id: 0x1B) else { return nil }
            return .measurementResult(uasValue)
        case .uas0x01:
            guard let uasValue = decodeUAS(data, id: 0x01) else { return nil }
            return .measurementResult(uasValue)
        case .uas0x16:
            guard let uasValue = decodeUAS(data, id: 0x16) else { return nil }
            return .measurementResult(uasValue)
        case .uas0x0B:
            guard let uasValue = decodeUAS(data, id: 0x0B) else { return nil }
            return .measurementResult(uasValue)
        case .uas0x1E:
            guard let uasValue = decodeUAS(data, id: 0x1E) else { return nil }
            return .measurementResult(uasValue)
        case .uas0x25:
            guard let uasValue = decodeUAS(data, id: 0x25) else { return nil }
            return .measurementResult(uasValue)
        case .uas0x27:
            guard let uasValue = decodeUAS(data, id: 0x27) else { return nil }
            return .measurementResult(uasValue)

        case .sensorVoltageBig:
            guard let voltage = sensorVoltageBig(data) else { return nil }
            return .measurementResult(voltage)

        case .evapPressure:
            guard let pressure = evapPressure(data) else { return nil }
            return .measurementResult(pressure)
        case .none:
            return nil
        case .absoluteLoad:
            guard let load = absoluteLoad(data) else { return nil }
            return .measurementResult(load)
        case .uas0x34:
            guard let uasValue = decodeUAS(data, id: 0x34) else { return nil }
            return .measurementResult(uasValue)
        case .maxMaf:
            guard let maf = maxMaf(data) else { return nil }
            return .measurementResult(maf)
        case .fuelType:
            guard let type = fuelType(data) else { return nil }
            return .stringResult(type)
        case .absEvapPressure:
            guard let pressure = absEvapPressure(data) else { return nil }
            return .measurementResult(pressure)
        case .evapPressureAlt:
            guard let pressure = evapPressureAlt(data) else { return nil }
            return .measurementResult(pressure)
        case .injectTiming:
            guard let timing = injectTiming(data) else { return nil }
            return .measurementResult(timing)
        case .dtc:
            guard let troubleCodes = dtc(data) else { return nil }
            return .troubleCode(troubleCodes)
        case .fuelRate:
            guard let rate = fuelRate(data) else { return nil }
            return .measurementResult(rate)
        case .monitor:
            guard let monitor = decodeMonitor(data) else { return nil }
        ...
        case .cvn:
            return nil
        case .encoded_string:
            guard let string = encoded_string(data) else { return nil }
