...
public struct CommandProperties {
    ...
    public func decode(data: Data, isMetric: MeasurementUnits = .metric) -> DecodeResult? {
        ...
        case .uas0x07:
            guard let measurement = decodeUAS(data, id: 0x07) else { return .none }
            return .measurementResult(measurement)
        case .temp:
            return decodeTemp(data)
        case .percent:
            guard let percent = percent(data) else { return .none }
            return .measurementResult(percent)
        case .currentCentered:
            guard let currentCentered = currentCentered(data) else { return .none }
            return .measurementResult(currentCentered)
        case .airStatus:
            guard let airStatus = currentCentered(data) else { return .none }
            return .measurementResult(airStatus)
        case .singleDTC:
            guard let dtc = singleDtc(data) else { return .none }
            return .troubleCode([dtc])

        case .fuelStatus:
            guard let fuelStatus = fuelStatus(data) else { return .none }
            return .stringResult(fuelStatus)

        case .percentCentered:
            guard let percentCentered = percent(data) else { return .none }
            return .measurementResult(percentCentered)
        case .fuelPressure:
            guard let fuelPressure = fuelPressure(data) else { return .none }
            return .measurementResult(fuelPressure)
        case .pressure:
            guard let pressure = pressure(data) else { return .none }
            return .measurementResult(pressure)
        case .timingAdvance:
            guard let timingAdvance = timingAdvance(data) else { return .none }
            return .measurementResult(timingAdvance)
            // TODO : add obd Compliance return type

        case .obdCompliance:
            guard let obdCompliance = obdCompliance(data) else { return .none }
            return .stringResult(obdCompliance)

        case .o2SensorsAlt:
            guard let o2SensorsAlt = o2SensorsAlt(data) else { return .none }
            return .stringResult(o2SensorsAlt)

        case .uas0x12:
            guard let uasValue =  decodeUAS(data, id: 0x12) else { return .none }
            return .measurementResult(uasValue)

        case .o2Sensors:
            guard let o2Sensors = o2Sensors(data) else { return .none }
            return .stringResult(o2Sensors)

        case .sensorVoltage:
            guard let voltage = voltage(data) else { return .none }
        ...
        case .uas0x19:
            guard let uasValue = decodeUAS(data, id: 0x19) else { return .none }
            return .measurementResult(uasValue)
        case .uas0x1B:
            guard let uasValue = decodeUAS(data, id: 0x1B) else { return .none }
            return .measurementResult(uasValue)
        case .uas0x01:
            guard let uasValue = decodeUAS(data, id: 0x01) else { return .none }
            return .measurementResult(uasValue)
        case .uas0x16:
            guard let uasValue = decodeUAS(data, id: 0x16) else { return .none }
            return .measurementResult(uasValue)
        case .uas0x0B:
            guard let uasValue = decodeUAS(data, id: 0x0B) else { return .none }
            return .measurementResult(uasValue)
        case .uas0x1E:
            guard let uasValue = decodeUAS(data, id: 0x1E) else { return .none }
            return .measurementResult(uasValue)
        case .uas0x25:
            guard let uasValue = decodeUAS(data, id: 0x25) else { return .none }
            return .measurementResult(uasValue)
        case .uas0x27:
            guard let uasValue = decodeUAS(data, id: 0x27) else { return .none }
            return .measurementResult(uasValue)

        case .sensorVoltageBig:
            guard let voltage = sensorVoltageBig(data) else { return .none }
            return .measurementResult(voltage)

        case .evapPressure:
            guard let pressure = evapPressure(data) else { return .none }
            return .measurementResult(pressure)
        case .none:
            return nil
        case .absoluteLoad:
            guard let load = absoluteLoad(data) else { return .none }
            return .measurementResult(load)
        case .uas0x34:
            guard let uasValue = decodeUAS(data, id: 0x34) else { return .none }
            return .measurementResult(uasValue)
        case .maxMaf:
            guard let maf = maxMaf(data) else { return .none }
            return .measurementResult(maf)
        case .fuelType:
            guard let type = fuelType(data) else { return .none }
            return .stringResult(type)
        case .absEvapPressure:
            guard let pressure = absEvapPressure(data) else { return .none }
            return .measurementResult(pressure)
        case .evapPressureAlt:
            guard let pressure = evapPressureAlt(data) else { return .none }
            return .measurementResult(pressure)
        case .injectTiming:
            guard let timing = injectTiming(data) else { return .none }
            return .measurementResult(timing)
        case .dtc:
            guard let troubleCodes = dtc(data) else { return .none }
            return .troubleCode(troubleCodes)
        case .fuelRate:
            guard let rate = fuelRate(data) else { return .none }
            return .measurementResult(rate)
        case .monitor:
            guard let monitor = monitor(data) else { return .none }
        ...
        case .cvn:
            return nil
        case .encoded_string:
            guard let string = encoded_string(data) else { return .none }
