...
public enum Decoders: Codable {
    ...
    static func dtc(_ data: Data) -> [String: String]? {
        ...
        for n in stride(from: 0, to: data.count - 1, by: 2) {
            ...
            codes.merge(dtc) { _, new in new }
        }
        return codes
    }

    static func singleDtc(_ data: Data) -> [String: String]? {
...
func parseDTC(_ data: Data) -> [String: String]? {
    ...
    let troubleCode = [dtc: codes[dtc] ?? "No description available."]
    return troubleCode
}
